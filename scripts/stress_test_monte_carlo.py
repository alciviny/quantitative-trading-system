...
def main():
    ...
    if not args.input_file.exists():
        logging.error(f"Arquivo de entrada não encontrado: {args.input_file}")
        sys.exit(1)
        
    args.output_dir.mkdir(parents=True, exist_ok=True)
    output_file = args.output_dir / "monte_carlo_results.csv"

    # --- Carregamento e Preparação dos Dados ---
    logging.info(f"Carregando resultados do backtest de: {args.input_file}")
    try:
        backtest_df = pd.read_csv(args.input_file)
