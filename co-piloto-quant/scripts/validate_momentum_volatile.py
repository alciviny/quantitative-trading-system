...
logger = logging.getLogger("validate_momentum_volatile")
...
def run_strategy_simulation(df: pd.DataFrame, strategy, ticker: str, target_regimes=None) -> pd.DataFrame:
    try:
    ...
    for i in range(1, len(df_eval)):
        ...
        if target_regimes and regime not in target_regimes:
            continue
...
def process_file_in_window(file_path, strategy, train_start, train_end, test_start, test_end, target_regimes):
    try:
        ...
        if not df_train.empty:
            trades_train = run_strategy_simulation(df_train, strategy, ticker, target_regimes)
            if not trades_train.empty:
                trades_train['phase'] = 'TRAIN'
        
        if not df_test.empty:
            trades_test = run_strategy_simulation(df_test, strategy, ticker, target_regimes)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Validacao MOMENTUM em VOLATILE regimes')
    parser.add_argument('--ema-fast', type=int, default=12)
    parser.add_argument('--ema-slow', type=int, default=26)
    parser.add_argument('--regime', type=str, default='VOLATILE')
    parser.add_argument('--workers', type=int, default=DEFAULT_WORKERS)
    parser.add_argument('--out', type=str, default='momentum_volatile_results.csv')
    args = parser.parse_args()
    
    target_regimes = ['BULL_' + args.regime, 'BEAR_' + args.regime]
    ...
    logger.info('Validacao: Walk-forward MOMENTUM em %s', '+'.join(target_regimes))
    ...
    for w_idx, window in enumerate(windows, 1):
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as ex:
            futures = {ex.submit(process_file_in_window, fp, strategy, 
                                ...
                                target_regimes): fp for fp in files}
    ...
    if all_trades:
        ...
        print('\n' + '=' * 80)
        print(f'MOMENTUM STRATEGY - VALIDACAO {args.regime} (2022-2025)')
        print('=' * 80)
        ...
        print(f'\n[DEGRADACAO]: {deg:.1f}%')
        if abs(deg) < 50:
            print('  -> EXCELENTE (consistente)')
        elif abs(deg) < 150:
            print('  -> BOM (razoavel)')
        else:
            print('  -> RUIM (overfitted ou teste favoravel)')
        ...
        for regime in sorted(final_df['regime'].unique()):
            ...
            if not train_df.empty and not test_df.empty:
                ...
                print(f'  {regime:15} | Train: {train_ret:+.4f} ({len(train_df):3}t) | Test: {test_ret:+.4f} ({len(test_df):3}t)')
