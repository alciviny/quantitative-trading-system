...
Walk-Forward com parâmetros OTIMIZADOS
- rsi_period: 60 (vs 120)
- max_half_life: 35 (vs 25)
- use_regime_filter: False (vs True)
- only_bull_market: False (vs default)
"""
...
def main():
    setup_logging()
    parser = argparse.ArgumentParser(description='Walk-Forward com PARÂMETROS OTIMIZADOS')
    ...
    parser.add_argument('--max-half-life', type=int, default=35)
    parser.add_argument('--workers', type=int, default=DEFAULT_WORKERS)
    parser.add_argument('--out', type=str, default='walk_forward_optimized_results.csv')
    ...
    strategy = MeanReversionStrategy(
        ...
        only_bull_market=False
    )

    logger.info('Estratégia: %s', strategy.get_name())
    logger.info('CONFIGURAÇÃO: 12 meses TREINO + 3 meses TESTE (OTIMIZADO)')
    logger.info('Parâmetros: RSI=%d, HL=%d, RegimeFilter=False, OnlyBull=False', 
                args.rsi_period, args.max_half_life)
    ...
    if all_trades:
        ...
        print('\n' + '=' * 70)
        print('WALK-FORWARD COM PARÂMETROS OTIMIZADOS')
        print('=' * 70)
        ...
        for phase in ['TRAIN', 'TEST']:
            ...
            if not phase_df.empty:
                ...
                print(f'  {phase:6} | {cnt:4} trades | {avg:8.4f} ret | {wr:6.1%} WR | std: {std:.4f}')
        ...
        for regime in sorted(final_df['regime'].unique()):
            regime_df = final_df[(final_df['regime'] == regime) & (final_df['phase'] == 'TRAIN')]
            if not regime_df.empty:
                avg = regime_df['return'].mean()
                wr = (regime_df['return'] > 0).mean()
                cnt = len(regime_df)
                print(f'  {regime:20} | {cnt:3} trades | {avg:8.4f} ret | {wr:6.1%} WR')
