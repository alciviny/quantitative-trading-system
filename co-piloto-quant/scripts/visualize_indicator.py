...
from co_piloto_quant.indicators.multi_bollinger_bands import multi_bollinger_bands
...
def visualize(
    ...
    # Unifica os desvios e cores para todas as bandas
    # Ordena de forma decrescente para a plotagem (de fora para dentro)
    sorted_deviations = sorted(bb_deviations, reverse=True)
    
    default_color = 'rgba(128, 128, 128, 0.2)'
    band_colors = [('rgba(0, 255, 0, 0.2)' if dev == 0.45 else default_color) for dev in sorted_deviations]

    # Calcula Bandas de Bollinger Múltiplas para o preço
    price_bands_df = multi_bollinger_bands(data['close'], period=bb_period, deviations=bb_deviations)
    price_bands_df = price_bands_df.add_prefix('price_') # Adiciona prefixo para evitar conflito
    ...
    obtr_system_df = calculate_system_tpm(data, indicator='obtr', period=system_period, deviations=bb_deviations)
    data = data.join(obtr_system_df)

    # Calcula System TPM para WAD
    print("Calculando System TPM para WAD...")
    wad_system_df = calculate_system_tpm(data, indicator='wad', period=system_period, deviations=bb_deviations)
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            f'Preço de {ticker} com Bandas de Bollinger Múltiplas ({bb_period}p)',
            f'IFR ({ifr_period}p)',
    ...
    # Subplot 1: Preço e Bandas de Bollinger Múltiplas
    ...
    # Plota as bandas de preço (agora com cores unificadas)
    for i, dev in enumerate(sorted_deviations):
        dev_str = str(dev).replace('.', '_')
        color = band_colors[i]
        fig.add_trace(go.Scatter(x=data.index, y=data[f'price_upper_band_{dev_str}'], mode='lines', line=dict(width=0), showlegend=False), row=1, col=1)
        fig.add_trace(go.Scatter(x=data.index, y=data[f'price_lower_band_{dev_str}'], mode='lines', line=dict(width=0), fill='tonexty', fillcolor=color, name=f'Banda {dev}σ'), row=1, col=1)

    fig.add_trace(go.Scatter(x=data.index, y=data['price_middle_band'], mode='lines', line=dict(dash='dash', color='darkgray', width=1), name='Média Central'), row=1, col=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bb_deviations", type=str, default="0.45,1.0,1.5,2.0,2.5", help="Desvios padrão para as Bandas de Bollinger do preço, separados por vírgula (ex: 1.5,2.0,2.5).")
    ...
    # Converte a string de desvios em uma lista de floats
    bb_devs = [float(dev) for dev in args.bb_deviations.split(',')]

    visualize(
        ticker=args.ticker,
        bb_period=args.bb_period,
        bb_deviations=bb_devs,
