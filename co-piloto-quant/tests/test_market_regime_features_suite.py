...
def test_realized_volatility_annualize():
    data = pd.Series(np.random.normal(100, 1, 120))
...
def test_volatility_of_volatility_positive():
    data = pd.Series(np.random.normal(100, 1, 120))
...
def test_ewma_volatility_lambda():
    data = pd.Series(np.random.normal(100, 1, 120))
...
def test_efficiency_ratio_limits():
    data = pd.Series(np.cumsum(np.random.normal(0, 1, 120)))
